...
class writer(object):
    ...
    def append(self, blob_name: str, blob: bytes, flush: int = 2):
        # XXX add a procmask MANDATORY argument to say which signals to mask
        """append a blob fully

        The file can be temporarily malformed during a call to append, so it's
        recommended to defer signals or other sources of interruption during the
        call.
